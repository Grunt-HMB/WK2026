...
from modules.settings import PREDICTION_OPTIONS, TOURNAMENT_START
...
def load_existing_predictions(user_id, predictions_df):
    ...
    user_preds = predictions_df[predictions_df["user_id"].astype(str) == str(user_id)]
...
def set_score(match_id, score1, score2):
    pred = result_from_score(score1, score2)
...
def show_score_dialog(match, match_id):
    team1 = str(match.get("team1", "Team 1"))
    team2 = str(match.get("team2", "Team 2"))
    current = st.session_state["local_predictions"].get(str(match_id), {})

    with st.expander(f"⚽ Uitslag invullen: {team1} - {team2}", expanded=True):
        c1, c2 = st.columns(2)
        ...
        if st.button("Score bevestigen", key=f"confirm_score_{match_id}", use_container_width=True):
...
""",
    ...
    b1, bx, b2, bs = st.columns([1, 1, 1, 2])
    ...
    with bs:
        if st.button("⚽ Uitslag", key=f"score_button_{match_id}", use_container_width=True, disabled=disabled):
            st.session_state[f"show_score_{match_id}"] = not st.session_state.get(f"show_score_{match_id}", False)
            st.rerun()
...
def show_group_phase(user, matches_df, predictions_df):
    ...
    groups = sorted([
        str(g) for g in matches_df["groep"].dropna().unique()
    ...
    with top1:
        st.markdown('<div class="main-title">👥 Groepsfase</div>', unsafe_allow_html=True)
        st.markdown('<div class="page-subtitle">Maak je voorspellingen per poule.</div>', unsafe_allow_html=True)

    with top2:
        selected_group = st.selectbox("Groep", groups, label_visibility="collapsed")
    ...
    group_matches = matches_df[matches_df["groep"].astype(str) == str(selected_group)].copy()
    group_matches = group_matches.sort_values(["datum", "tijd", "match_id"], kind="stable")
    ...
    st.markdown('<div class="info-box">ℹ️ Je kan tussen poules wisselen. Gebruik Concept opslaan om je keuzes veilig te bewaren.</div>', unsafe_allow_html=True)

    c1, c2 = st.columns([1, 1])

    with c1:
        if st.button("💾 Concept opslaan", use_container_width=True, disabled=disabled):
            count = batch_upsert_predictions(user_id, st.session_state["local_predictions"], "DRAFT")
            st.success(f"{count} keuzes opgeslagen als concept.")
            st.rerun()

    with c2:
        if st.button("✅ Definitief indienen", use_container_width=True, disabled=disabled):
            count = batch_upsert_predictions(user_id, st.session_state["local_predictions"], "FINAL")
