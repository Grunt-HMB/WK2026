...
def show_pronostiek_scores(user_id="Tom"):
    ...
    def render_score_control(match_id, side, team_name, flag, current_score, other_score):
        st.markdown(
            f"""
<div class="team-score-label">{flag} {team_name}</div>
            """,
            unsafe_allow_html=True,
        )

        c_minus, c_value, c_plus = st.columns([1, 1, 1], gap="small")

        with c_minus:
            if st.button(
                "−",
                key=f"minus_{side}_{match_id}",
                use_container_width=True,
            ):
                if side == "home":
                    set_score(match_id, max(current_score - 1, 0), other_score)
                else:
                    set_score(match_id, other_score, max(current_score - 1, 0))
                st.rerun()

        with c_value:
            st.markdown(
                f"""
<div class="score-display">{current_score}</div>
                """,
                unsafe_allow_html=True,
            )
        ...
        with c_plus:
            if st.button(
                "+",
                key=f"plus_{side}_{match_id}",
                use_container_width=True,
            ):
                if side == "home":
                    set_score(match_id, current_score + 1, other_score)
                else:
                    set_score(match_id, other_score, current_score + 1)
                st.rerun()
...
{country_flag(team2_code)} {team2}
            ...
            c_home, c_away = st.columns(2, gap="small")

            with c_home:
                render_score_control(
                    match_id=match_id,
                    side="home",
                    team_name=team1,
                    flag=country_flag(team1_code),
